...
## Notes
- **Post-ship bug fix (user-reported live failure):** popup Scan threw "Could not establish connection. Receiving end does not exist." Root cause: the content-script listener was absent in tabs opened before the extension loaded, or reached via YouTube SPA navigation (declarative content_scripts don't re-inject on History API nav). Fix: popup now injects the content script on demand via `chrome.scripting.executeScript` then retries (`src/popup/scan-request.ts`), and the content script guards init with `window.__coordcardReady` so re-injection never double-registers. Test-first regression: `tests/popup-scan.spec.ts` (RED→GREEN). 16 specs / 95 assertions green.
